...
import {
  ...
  GameNavLink,
  GameNavTimer,
} from "@/platform/layout/GameNav";
import { Tile } from "../ui/Tile";
import { EternityLogo } from "../ui/EternityLogo";
import { Tutorial, type HighlightTarget } from "../ui/Tutorial";
...
import { loadSave, storeSave } from "../logic/saves";
import { isTutorialCompleted, markTutorialCompleted } from "../logic/progress";
import { ETERNITY2_CLUE } from "../logic/pieces-original";
import {
  createLevel,
  emptyBoard,
  conflictsAt,
...
const scores = createScoresService("scores_eternity_ii");

const EMPTY_LEVEL: Level = { size: 4, tiles: [], fixed: [], original: false };

export function PlayPage() {
  const { t } = useI18n();
  const { level: size, resume } = useSearch({ from: "/$lang/eternity-ii/play" });
  const { user } = useAuth();
  ...
  const edgePieceIds = useMemo(() => {
    return level.tiles.filter((tile) => tile.edges.some((e) => e === 0)).map((tile) => tile.id);
  }, [level.tiles]);
        ...
        <header className="text-center">
          ...
          <div className="mt-2 flex justify-center">
            <button
              ...
              className="inline-flex cursor-pointer items-center gap-1.5 rounded-full px-3 py-1 text-xs transition-colors hover:underline"
              style={{
                color: "var(--e2-ink)",
                ...
                <button
                  ...
                  className={`e2-btn inline-flex items-center gap-1.5 ${
                    highlightedElement === "rotate"
                      ? "ring-4 ring-[var(--e2-accent)] animate-pulse"
                      : ""
                  }`}
        ...
        {!level.original && (
          <>
            ...
            <GameNavButton
              ...
              label={
                isBruteForcing
                  ? `${t("e2.bruteForce")} (${bruteForceProgress})`
                  : t("e2.bruteForce")
              }
            />
            <GameNavButton
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-4">
          ...
          >
            ...
            {helped ? (
              ...
              <ScoreForm service={scores} level={level.size} seconds={seconds} />
            )}
