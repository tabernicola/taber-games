...
import { isMeowdokuSolved, meowdokuConflicts, type MeowdokuPuzzle } from "../logic/meowdoku";
...
export function MeowdokuBoard({
  ...
  const solved = isMeowdokuSolved(puzzle, occupied);
  const { seconds } = useTimer(!solved);
  const conflicts = useMemo(() => meowdokuConflicts(puzzle, occupied), [puzzle, occupied]);

  const charAt = (cell: number) =>
    cast.find((c) => placements[c.id] === cell) ?? undefined;
    ...
    <div className="murdoku-light min-h-screen">
      <header className="sticky top-0 z-30 flex items-center justify-between border-b border-border bg-background/80 px-3 py-2 backdrop-blur">
        ...
        <h1 className="text-base font-bold tracking-widest text-primary">
          {t("murdoku.mode.meowdoku")} · {size}×{size}
        </h1>
