...
export function* bruteForceSolver(
  level: Level,
  board: Placement[],
): Generator<{ board: Placement[]; index: number; placed: boolean }, void, unknown> {
  const size = level.size;
  ...
  function getCachedCandidates(cell: number, tiles: Tile[]): Map<number, Rotation> {
    const cacheKey = `${cell}-${tiles.map(t => t.id).sort().join(',')}`;
  ...
  function* solveRecursive(availableTiles: Tile[]): Generator<{ board: Placement[]; index: number; placed: boolean }, boolean, unknown> {
